...
export default function Admin() {
  ...
  async function loadEvents() {
    ...
    const { data, error } = await applyTownFilter(
      supabase.from('events').select('*').eq('status', filter)
  ...
  function townUrl(path: string) {
    const town = selectedTown || adminTowns?.[0]
    return !isSuperAdmin && town ? `${path}?town=${encodeURIComponent(town)}` : path
  }
  ...
  if (editingEvent) return (
    <div style={{ minHeight: '100vh', background: '#fafaf8', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
          {[{ value: 'free', label: 'Free' }, { value: 'family', label: 'Family-Friendly' }, { value: 'wellness', label: 'Wellness' }, { value: 'reg', label: 'Reg. Required' }, { value: 'music', label: 'Live Music' }].map(({ value: tag, label: tagLabel }) => {
    ...
    <div style={{ minHeight: '100vh', background: '#fafaf8', fontFamily: 'sans-serif' }}>
      ...
      <header style={{ background: '#1a3d2b', padding: '14px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <span style={{ fontWeight: 800, fontSize: '22px', color: 'white', letterSpacing: '-1px' }}>town</span>
          <span style={{ color: '#7EC8A4', fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: '22px', fontWeight: 400 }}>stir</span>
          <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginLeft: '12px' }}>{isSuperAdmin ? 'Super Admin' : `Admin${selectedTown ? ` — ${selectedTown}` : ''}`}</span>
        </div>
        <div style={{ display: 'flex', gap: '10px' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        {!isSuperAdmin && adminTowns && adminTowns.length > 1 && (
          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
            ...
            {adminTowns.map(t => (
              <button key={t} onClick={() => setSelectedTown(t)} style={{ padding: '6px 16px', borderRadius: '999px', border: '1.5px solid', borderColor: selectedTown === t ? '#1a3d2b' : '#e5e7eb', background: selectedTown === t ? '#1a3d2b' : 'white', color: selectedTown === t ? 'white' : '#6b7280', fontWeight: 600, fontSize: '12px', cursor: 'pointer' }}>{t}</button>
            ))}
          </div>
        )}
        ...
        {filter === 'organizations' && (
          <>
            {loading ? <div style={{ textAlign: 'center', padding: '40px', color: '#9ca3af' }}>Loading…</div>
              : orgs.length === 0 ? <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9ca3af' }}><p>No organizations found.</p></div>
              : (
                <>
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
                    <button onClick={() => setMessageModal({ all: true })} style={{ background: '#1a3d2b', color: 'white', border: 'none', padding: '9px 22px', borderRadius: '999px', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}>Message All Orgs</button>
                  </div>
                  {orgs.map(org => (
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div>
                            ...
                            <div style={{ marginTop: '4px' }}>
                              <span onClick={() => { setFilter('approved'); setFilterOrg(org.name) }} style={{ fontSize: '12px', color: orgEventCounts[org.id] > 0 ? '#1a3d2b' : '#9ca3af', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}>
        ...
        {filter !== 'organizations' && filter !== 'opportunities' && (
          <>
            ...
            {loading ? <div style={{ textAlign: 'center', padding: '40px', color: '#9ca3af' }}>Loading…</div>
              : events.length === 0 ? <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9ca3af' }}><p>No {filter} events.</p></div>
              : events.filter(ev => {
                ...
                <div key={ev.id} style={{ background: 'white', borderRadius: '12px', padding: '20px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderLeft: `4px solid ${selected.has(ev.id) ? '#1a3d2b' : '#e5e7eb'}` }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    {filter === 'pending' && <input type="checkbox" checked={selected.has(ev.id)} onChange={() => toggleSelect(ev.id)} style={{ width: '16px', height: '16px', cursor: 'pointer', marginRight: '12px', marginTop: '3px', flexShrink: 0 }} />}
                    <div style={{ flex: 1 }}>
