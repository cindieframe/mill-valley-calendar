...
function buildFeatured(data: any[]): any[] {
  // Step 1: events with their own image
  const withImages = data.filter(e => e.image_url)

  // Step 2: events without image — deduplicate by stock photo
  // Only one event per stock photo category allowed
  const usedStockCats = new Set<string>()
  const withStock: any[] = []

  // Sort no-image events: weekend first, then by category priority
  const noImage = data
  ...
  return [...withImages, ...withStock].slice(0, 4)
}
